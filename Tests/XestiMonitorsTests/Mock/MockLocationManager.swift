...
internal class MockLocationManager: LocationManagerProtocol {
    static func deferredLocationUpdatesAvailable() -> Bool {
        mockDeferredLocationUpdatesAvailable
    }

#if os(iOS) || os(macOS) || os(watchOS)
    static func headingAvailable() -> Bool {
        mockHeadingAvailable
    }
#endif
    ...
    static func significantLocationChangeMonitoringAvailable() -> Bool {
        mockSignificantLocationAvailable
    }
#endif
    ...
    func pauseStandardLocationUpdates() {
        guard mockStandardLocationRunning
        else { return }
    ...
    func updateAuthorization(error: any Error) {
    ...
    func updateAuthorization(forceStatus: CLAuthorizationStatus) {
        Self.mockAuthorizationStatus = forceStatus
    }
    ...
    func updateBeaconRanging(available: Bool) {
        Self.mockBeaconRangingAvailable = available
    }
#endif

#if os(iOS) || os(macOS)
    func updateBeaconRanging(error: any Error) {
        guard !rangedRegions.isEmpty
        else { return }
    ...
    func updateBeaconRanging(error: any Error,
        ...
        guard rangedRegions.contains(region)
        else { return }
    ...
    func updateHeading(available: Bool) {
        Self.mockHeadingAvailable = available
    }
#endif

#if os(iOS) || os(macOS) || os(watchOS)
    func updateHeading(error: any Error) {
        guard mockHeadingRunning
        else { return }
    ...
    func updateLocationServices(enabled: Bool) {
        Self.mockLocationServicesEnabled = enabled
    }
    ...
    func updateRegion(_ region: CLRegion?,
        ...
        if let region = region {
            guard monitoredRegions.contains(region)
            else { return }
        }
    ...
    func updateRegion(available: Bool) {
        Self.mockRegionAvailable = available
    }
#endif
...
#if os(iOS) || os(macOS)
    func updateRegion(error: any Error) {
        guard !monitoredRegions.isEmpty
        else { return }
    ...
    func updateRegion(state: CLRegionState) {
        guard let region = mockRegionStateRequested
        else { return }
    ...
    func updateSignificantLocation(_ location: CLLocation) {
        guard mockSignificantLocationRunning
        else { return }
    ...
    func updateSignificantLocation(available: Bool) {
        Self.mockSignificantLocationAvailable = available
    }
#endif

#if os(iOS) || os(macOS)
    func updateSignificantLocation(error: any Error) {
        guard mockSignificantLocationRunning
        else { return }
    ...
    func updateStandardLocation(canDeferUpdates: Bool) {
        Self.mockDeferredLocationUpdatesAvailable = canDeferUpdates
    }
#endif

#if os(iOS) || os(macOS)
    func updateStandardLocation(deferredError: (any Error)?) {
        guard mockStandardLocationRunning
        else { return }
    ...
    func updateStandardLocation(error: any Error) {
        guard mockStandardLocationRunning
        else { return }
...
#if os(iOS) || os(macOS)
    func updateVisit(error: any Error) {
        guard mockVisitRunning
        else { return }
