...
internal class MotionActivityMonitorTests: XCTestCase {
    ...
    override func setUp() {
        ...
        DependencyResolver.register(motionActivityManager as any MotionActivityManagerProtocol)
    }

    func testIsAvailable_false() {
        let monitor = MotionActivityMonitor { _ in
    ...
    func testMonitor_data() {
        ...
        let monitor = MotionActivityMonitor { event in
            XCTAssertEqual(OperationQueue.current, .main)
