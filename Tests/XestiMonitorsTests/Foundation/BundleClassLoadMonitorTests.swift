...
internal class BundleClassLoadMonitorTests: XCTestCase {
    ...
    func testMonitor_contentSizeDidChange_badUserInfo() {
        ...
        let monitor = BundleClassLoadMonitor(bundle: bundle) { _ in
    ...
    func testMonitor_didLoad() {
        ...
        let monitor = BundleClassLoadMonitor(bundle: bundle) { event in
            XCTAssertEqual(OperationQueue.current, .main)
        ...
        monitor.startMonitoring()
        _simulateDidLoad(with: expectedLoadedClasses)
        waitForExpectations(timeout: 1)
        monitor.stopMonitoring()
    ...
    private func _simulateDidLoad(with loadedClasses: [String],
